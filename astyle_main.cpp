 ...
 */
void ASConsole::formatCinToCout(string input, ostream& cout)
{
	...
	//istream* inStream = &input;
	stringstream outStream (input);
	/*char ch;
	inStream->get(ch);
...
extern "C" char* process(char* code, char* style, char* indent, char* lang)
{
	// create objects
	ASFormatter formatter;
	...
	vector<string> argvOptions;
	argvOptions.push_back(string(style));
	argvOptions.push_back(string(indent));
	argvOptions.push_back(string(lang));
	...
	//else
	console->formatCinToCout(string(code), oss);
	if (buf != NULL)
	   free(buf);
	buf = (char*) malloc(oss.str().length() + 1);
